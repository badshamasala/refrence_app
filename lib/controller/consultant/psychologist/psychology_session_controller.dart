...
class PsychologySessionController extends GetxController
  ...
  Future<void> getUpcomingSessions() async {
    try {
      ...
      CoachUpcomingSessionsModel? response = await CoachService()
          .upcomingSessions(globalUserIdDetails!.userId!, "Psychologist");
      if (response != null &&
  ...
  Future<void> getPastSessions() async {
    try {
      ...
      CoachPastSessionsModel? response = await CoachService()
          .pastSessions(globalUserIdDetails!.userId!, "Psychologist");
  ...
  Future<void> getSessionSummary() async {
    try {
      ...
      CoachSessionSummaryModel? response = await CoachService()
          .getSessionsSummary(globalUserIdDetails!.userId!, "Psychologist");
  ...
  joinCallViaAgora(
    ...
    if (joinCallDetails.agoraDetails!.token != null) {
      ...
      EventsService().sendEvent("Psychologist_Session_Call_Join", {
        "session_id": joinCallDetails.session!.sessionId,
        "coach_id": joinCallDetails.coach!.coachId!,
        "coach_name": joinCallDetails.coach!.coachName!,
      ...
      EventsService().sendClickNextEvent(
          "PsychologistSessions", "Session", "VideoCallAgora");
  ...
  getPendingReviews() async {
    try {
      ...
      CoachPendingReviewsModel? response = await CoachService()
          .getPendingReviews(globalUserIdDetails!.userId!, "Psychologist");
      ...
      update(["PsychologistSessionReviews"]);
    }
  }
}
